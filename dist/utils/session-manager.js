...
export class SessionManager {
    ...
    saveSession(name, chatHistory, context, description) {
    ...
    deleteSession(sessionIdOrName) {
        const sessions = this.listSessions();
        const session = sessions.find(s => s.id === sessionIdOrName || s.name === sessionIdOrName);
        if (!session) {
            return false;
        }
    ...
    exportSessionToMarkdown(sessionIdOrName, outputPath) {
    ...
    updateSessionMetadata(sessionIdOrName, updates) {
        const sessionData = this.loadSession(sessionIdOrName);
        if (!sessionData) {
            return false;
        }
