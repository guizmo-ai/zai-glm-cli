...
export interface TaskMetrics {
  ...
  firstAttemptSuccess: boolean; // Did first tool call succeed?
...
export class MetricsCollector {
  ...
  private currentTask: Partial<TaskMetrics> | null = null;
  ...
  endTask(completed: boolean): void {
    if (this.currentTask) {
      ...
      this.currentTask.durationMs =
        ...
        (this.currentTask.startTime as Date).getTime();

      this.metrics.push(this.currentTask as TaskMetrics);
      this.saveMetrics();
  ...
  getRecentMetrics(count: number = 10): TaskMetrics[] {
  ...
  getToolUsageStats(): Record<
    string,
    { count: number; percentage: number; avgPerTask: number }
  > {
  ...
  getPerformanceTrends(windowSize: number = 10): {
    ...
    const calculateAvg = (metrics: TaskMetrics[]) => {
    ...
    return {
      ...
      improvement: {
        toolRounds:
          ((previous.avgToolRounds - recent.avgToolRounds) /
            previous.avgToolRounds) *
          100,
        errors:
          ((previous.avgErrors - recent.avgErrors) / previous.avgErrors) * 100,
        duration:
          ((previous.avgDuration - recent.avgDuration) / previous.avgDuration) *
          100,
  ...
  exportMetrics(filePath?: string): string {
    const exportPath =
      filePath || path.join(this.metricsDir, `metrics-export-${Date.now()}.json`);
