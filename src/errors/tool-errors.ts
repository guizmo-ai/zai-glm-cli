...
export class BashCommandError extends ZAIError {
  constructor(
    ...
    super(
      `Bash command failed with exit code ${exitCode}: ${command}`,
      'BASH_COMMAND_ERROR',
      {
        recoverable: true,
...
export class DirectoryNotFoundError extends FileOperationError {
  constructor(dirPath: string, operation: string = 'access') {
    super(
      `Directory not found: ${dirPath}`,
      { directory: dirPath, operation },
      [
        {
...
export class InvalidLineNumberError extends FileOperationError {
  constructor(
    ...
    super(
      `Invalid line number ${lineNumber}: File has ${totalLines} lines`,
      { file: filePath, lineNumber, totalLines, operation },
      [
        {
